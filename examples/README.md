...
Credential-free example output is checked by running each example and comparing its normalized output with the snapshots in `tests/snapshots/`. Random action hashes and process IDs are replaced with placeholders; examples still execute during the test.

Run `python -m pytest tests/test_example_snapshots.py` to check the snapshots. After intentionally changing example output, run `python -m pytest tests/test_example_snapshots.py --update-example-snapshots`, then review the snapshot diff before committing it. This command runs the examples and rewrites their expected output; it does not copy recorded fixtures over the example execution.
