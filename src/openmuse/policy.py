...
class Policy:
    def __init__(self, allow_writes=False, approvals: ApprovalAuthority | None = None):
    ...
    def check(self, risk: Risk, action: Action) -> Decision:
        ...
        if self.approvals and self.approvals.verify(action, action.approval_token):
            return Decision(True, "valid one-time approval")
