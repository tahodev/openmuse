...
class OpenAICompatiblePlanner:
    def __init__(self,base_url="https://api.openai.com/v1",model="gpt-4.1-mini",api_key=None,max_response_bytes=1_000_000):
        self.base_url=base_url.rstrip("/"); self.model=model; self.api_key=api_key or os.environ.get("OPENAI_API_KEY"); self.max_response_bytes=max_response_bytes; self.last_reply: str | None = None
        if not self.api_key: raise ValueError("OpenAI-compatible API key is required")
    def plan(self,goal,tools,history):
        self.last_reply=None
        prompt={"goal":goal,"tools":tools,"history":[r.__dict__ for r in history],"instruction":"Return JSON {tool,arguments} to act, or {done:true,reply} to finish; reply is an optional short answer for the user. Never output approval_token."}
        ...
        if value.get("done") is True:
            reply=value.get("reply"); self.last_reply=reply if isinstance(reply,str) else None
            return None
